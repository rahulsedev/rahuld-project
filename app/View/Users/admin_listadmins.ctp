...
<div class="container-fluid"> <!-- container-fluid start -->
    <div class="pull-right user-btn">
        <?php echo $this->Html->link('Add New Administrator', array('controller' => 'users', 'action' => 'addadmin', 'admin' => true), array('escape' => false, 'class' => 'btn btn-info', 'title' => 'Add New User', 'id' => 'addAdmin')); ?>
        &nbsp;<?php echo $this->Html->link('Trash', array('controller' => 'users', 'action' => 'trashadmins', 'admin' => true), array('escape' => false, 'class' => 'btn btn-info', 'title' => 'Deleted Administrators')); ?>
    </div>
    <div class="clearfix"></div>	
    <div class="panel panel-default table-margin">
        <div class="panel-heading">
            ...
            <div class="pull-right">
                ...
                echo $this->Form->input('User.search', array('type' => 'text', 'value' => $val, 'placeholder' => "Search Administrators", 'class' => 'search form-control', 'maxlength' => 100, 'label' => false, 'div' => false));
                ?>
                ...
                <table class="table table-bordered table-striped">
                    <thead>
                        <tr>
                            ...
                            <th><?php echo $this->Paginator->sort('UserType.name', 'User Type'); ?></th>
                        ...
                        if (isset($data) && !empty($data)) {
                            foreach ($data as $key => $processData) {
                                ...
                                <tr>
                                    ...
                                    <td><?php echo $processData['UserType']['name']; ?></td>
                                    <td><?php echo ($processData['User']['status'] == 1) ? '<span class="label label-success">Active</span>' : '<span class="label label-danger">Inactive</span>'; ?></td>
                                    <td><?php echo date(Configure::read('DISPLAY_DATETIME.DateTime'), strtotime($processData['User']['modified'])); ?></td>
                                    <td>
                                        <?php echo $this->Html->link('<i class="glyphicon glyphicon-search text-color"></i>', array('controller' => 'users', 'action' => 'viewadmin', base64_encode($processData['User']['id']), 'admin' => true), array('escape' => false, 'class' => 'btn btn-sm btn-default viewUserRow', 'title' => 'View Administrator Details',)); ?>
                                        <?php echo $this->Html->link('<i class="glyphicon glyphicon-pencil text-color"></i>', array('controller' => 'users', 'action' => 'editadmin', base64_encode($processData['User']['id'])), array('escape' => false, 'class' => 'btn btn-sm btn-default editRow', 'title' => 'Edit User')); ?>                                    
                                    <?php if($processData['User']['user_type_id']!=1){ ?>    
                                        <?php
                                        if ($processData['User']['status'] == 1) {
                                            echo $this->Html->link('<i class="glyphicon glyphicon-ok-circle"></i>', array('controller' => 'users', 'action' => 'blockadmin', base64_encode($processData['User']['id']), 'block'), array('data-username' => $processData['User']['first_name'] . ' ' . $processData['User']['last_name'], 'escape' => false, 'class' => 'btn btn-sm btn-default block', 'title' => 'Deactivate Administrator'));
                                        } else {
                                            echo $this->Html->link('<i class="glyphicon glyphicon-ban-circle"></i>', array('controller' => 'users', 'action' => 'blockadmin', base64_encode($processData['User']['id']), 'unblock'), array('data-username' => $processData['User']['first_name'] . ' ' . $processData['User']['last_name'], 'escape' => false, 'class' => 'btn btn-sm btn-default block', 'title' => 'Activate Administrator'));
                                        }
                                        ?>
                                        <?php echo $this->Html->link('<i class="glyphicon glyphicon-trash text-color"></i>', 'javascript:void(0)', array('data-name' => $processData['User']['first_name'] . ' ' . $processData['User']['last_name'], 'escape' => false, 'class' => 'btn btn-sm btn-default delete', 'title' => 'Delete User', 'data-href' => $this->Html->url(array('controller' => 'users', 'action' => 'deleteadmin', base64_encode($processData['User']['id']))))); ?>
                                    <?php } ?>
                                    </td> 
                                </tr>
                                <?php
