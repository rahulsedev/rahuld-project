...
<div class="container-fluid"> <!-- container-fluid start -->
    <div class="pull-right user-btn">
        <?php echo $this->Html->link('Add New', array('controller' => 'clients', 'action' => 'addcmaster', 'admin' => true), array('escape' => false, 'class' => 'btn btn-info', 'title' => 'Add New User', 'id' => 'addAdmin')); ?>
    </div>
    <div class="clearfix"></div>	
    <div class="panel panel-default table-margin">
        <div class="panel-heading">
            ...
            <div class="pull-right">
                <?php echo $this->Form->create('clients', array('action' => 'listcmasters', 'type' => "GET"), array('class' => "control-group")); ?>
                <?php
                ...
                <?php
                    echo $this->Form->input('User.search', array('type' => 'text', 'value' => $val, 'placeholder' => "Search Keyword", 'class' => 'search form-control', 'maxlength' => 100, 'label' => false, 'div' => false, 'style'=>'margin-left:4px;'));
                ?>
            ...
            <div class="table-responsive">                                                         
                <table class="table table-bordered table-striped">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th><?php echo $this->Paginator->sort('Client.name', 'Client Name'); ?></th>
                            <th><?php echo $this->Paginator->sort('material', 'Material'); ?></th>
                            <th><?php echo $this->Paginator->sort('quantity', 'Quantity'); ?></th>
                            <th><?php echo $this->Paginator->sort('invoice_no', 'Invoice #'); ?></th>
                        ...
                        if (isset($data) && !empty($data)) {
                            foreach ($data as $key => $processData) {
                                ...
                                <tr>
                                    ...
                                    <td><?php echo $processData['ClientMaster']['material']; ?></td>
                                    <td align="center"><?php echo $processData['ClientMaster']['quantity']; ?></td>
                                    <td align="center"><?php echo $processData['ClientMaster']['invoice_no']; ?></td>
                                    <td><?php echo date("d-m-Y", strtotime($processData['ClientMaster']['added_date'])); ?></td>
                                    <td><?php echo $processData['User']['full_name']."(".$processData['UserType']['name'].")"; ?></td>
                                    <!--td><?php // echo ($processData['ClientMaster']['status'] == 1) ? '<span class="label label-success">Active</span>' : '<span class="label label-danger">Inactive</span>'; ?></td-->
                                    <td><?php echo date(Configure::read('DISPLAY_DATETIME.DateTime'), strtotime($processData['ClientMaster']['created'])); ?></td>
                                    <td>
                                        <?php echo $this->Html->link('<i class="glyphicon glyphicon-search text-color"></i>', array('controller' => 'clients', 'action' => 'viewcmaster', base64_encode($processData['ClientMaster']['id']), 'admin' => true), array('escape' => false, 'class' => 'btn btn-sm btn-default viewUserRow', 'title' => 'View master details',)); ?>
                                        <?php // echo $this->Html->link('<i class="glyphicon glyphicon-pencil text-color"></i>', array('controller' => 'users', 'action' => 'editadmin', base64_encode($processData['ClientMaster']['id'])), array('escape' => false, 'class' => 'btn btn-sm btn-default editRow', 'title' => 'Edit User')); ?>				
                                        <?php
                                        /*if ($processData['ClientMaster']['status'] == 1) {
                                            echo $this->Html->link('<i class="glyphicon glyphicon-ok-circle"></i>', array('controller' => 'users', 'action' => 'blockadmin', base64_encode($processData['ClientMaster']['id']), 'block'), array('data-username' => $processData['ClientMaster']['first_name'] . ' ' . $processData['ClientMaster']['last_name'], 'escape' => false, 'class' => 'btn btn-sm btn-default block', 'title' => 'Deactivate Administrator'));
                                        } else {
                                            echo $this->Html->link('<i class="glyphicon glyphicon-ban-circle"></i>', array('controller' => 'users', 'action' => 'blockadmin', base64_encode($processData['ClientMaster']['id']), 'unblock'), array('data-username' => $processData['ClientMaster']['first_name'] . ' ' . $processData['ClientMaster']['last_name'], 'escape' => false, 'class' => 'btn btn-sm btn-default block', 'title' => 'Activate Administrator'));
                                        }*/
                                        ?>
                                        <?php // echo $this->Html->link('<i class="glyphicon glyphicon-trash text-color"></i>', 'javascript:void(0)', array('data-name' => $processData['ClientMaster']['first_name'] . ' ' . $processData['ClientMaster']['last_name'], 'escape' => false, 'class' => 'btn btn-sm btn-default delete', 'title' => 'Delete User', 'data-href' => $this->Html->url(array('controller' => 'users', 'action' => 'deleteadmin', base64_encode($processData['ClientMaster']['id']))))); ?>
                                    </td> 
                                </tr>
                                <?php
                            }
...
<?php echo $this->Element('admin/addClientMaster'); ?>
<!-- Modal Edit Client -->
<?php echo $this->Element('admin/viewClientMaster'); ?>
