...
<div id="breadcrumbs" class="breadcrumbs clearfix"> <!-- breadcrumbs start -->
    <ul class="breadcrumb col-sm-6">
        <li>
            ...
            <?php echo $this->Html->link('Home', array('controller' => 'clients', 'action' => 'dashboard', 'admin' => true)); ?>
        </li>
...
<div class="container-fluid"> <!-- container-fluid start -->
    <div class="pull-right user-btn">
        <?php echo $this->Html->link('Add New', array('controller' => 'clients', 'action' => 'editclient/0', 'admin' => true), array('escape' => false, 'class' => 'btn btn-info editRow', 'title' => 'Add New User', 'id' => 'addAdmin')); ?>
    </div>
    <div class="clearfix"></div>	
    <div class="panel panel-default table-margin">
        <div class="panel-heading">
            ...
            <div class="pull-right">
                <?php echo $this->Form->create('clients', array('action' => 'listclients', 'type' => "GET"), array('class' => "control-group")); ?>
                <?php
                ...
                <?php
                echo $this->Form->input('User.search', array('type' => 'text', 'value' => $val, 'placeholder' => "Search Client", 'class' => 'search form-control', 'maxlength' => 100, 'label' => false, 'div' => false, 'style'=>'margin-left:4px;'));
                ?>
                        ...
                        if (isset($data) && !empty($data)) {
                            foreach ($data as $key => $processData) {
                                ...
                                <tr>
                                    ...
                                    <td><?php echo $processData['Client']['name']; ?></td>
                                    <td><?php echo $processData['Client']['code']; ?></td>
                                    <td><?php echo $processData['Client']['email']; ?></td>
                                    <td align="center"><?php echo $processData['Client']['pan_no']; ?></td>
                                    <td><?php echo date("d-m-Y", strtotime($processData['Client']['add_date'])); ?></td>
                                    <td><?php echo $processData['User']['full_name']."(".$processData['UserType']['name'].")"; ?></td>
                                    <td><?php echo ($processData['Client']['status'] == 1) ? '<span class="label label-success">Active</span>' : '<span class="label label-danger">Inactive</span>'; ?></td>
                                    <td><?php echo date(Configure::read('DISPLAY_DATETIME.DateTime'), strtotime($processData['Client']['created'])); ?></td>
                                    <td>
                                        <?php //echo $this->Html->link('<i class="glyphicon glyphicon-search text-color"></i>', array('controller' => 'clients', 'action' => 'viewmaster', base64_encode($processData['Client']['id']), 'admin' => true), array('escape' => false, 'class' => 'btn btn-sm btn-default viewUserRow', 'title' => 'View master details',)); ?>`
                                        <?php echo $this->Html->link('<i class="glyphicon glyphicon-pencil text-color"></i>', array('controller' => 'clients', 'action' => 'editclient', base64_encode($processData['Client']['id'])), array('escape' => false, 'class' => 'btn btn-sm btn-default editRow', 'title' => 'Edit User')); ?>				
                                        <?php
                                        if ($processData['Client']['status'] == 1) {
                                            echo $this->Html->link('<i class="glyphicon glyphicon-ok-circle"></i>', array('controller' => 'clients', 'action' => 'blockadmin', base64_encode($processData['Client']['id']), 'block'), array('data-username' => $processData['Client']['name'], 'escape' => false, 'class' => 'btn btn-sm btn-default block', 'title' => 'Deactivate Client'));
                                        } else {
                                            echo $this->Html->link('<i class="glyphicon glyphicon-ban-circle icon-color"></i>', array('controller' => 'clients', 'action' => 'blockadmin', base64_encode($processData['Client']['id']), 'unblock'), array('data-username' => $processData['Client']['name'], 'escape' => false, 'class' => 'btn btn-sm btn-default block', 'title' => 'Activate Client'));
                                        }
                                        ?>
                                        <?php echo $this->Html->link('<i class="glyphicon glyphicon-trash text-color"></i>', 'javascript:void(0)', array('data-name' => $processData['Client']['name'], 'escape' => false, 'class' => 'btn btn-sm btn-default delete', 'title' => 'Delete Client', 'data-href' => $this->Html->url(array('controller' => 'clients', 'action' => 'deleteadmin', base64_encode($processData['Client']['id']))))); ?>
                                    </td> 
                                </tr>
                                <?php
                            }
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                Are you sure want to <span>Active/Inactive</span> "<strong>selected record</strong>" from clients list?	
            </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3>Trusted/Untrusted Client</h3>
            </div>
            <div class="modal-body">
                <center>
                    Are you sure want to <span>trusted</span> "<strong>selected user</strong>" from clients list?	
                </center>	
            </div>
            <div class="modal-footer">
                <center>
    ...
    $(document).ready(function() {
        ...
        $(".block").click(function(event) {
            ...
            if ($(this).children('i').hasClass('icon-color')) {
                ...
                var bl = 'deactivate';
                var title = 'Deactivate Client';
            }
            $message = 'Are you sure want to ' + bl + ' "<strong>' + $(this).attr('data-username') + '</strong>" from clients list?';
