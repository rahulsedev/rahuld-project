    ...
    <div class="panel panel-default table-margin">
        <div class="panel-heading">
            ...
            <div class="pull-right">
                <?php echo $this->Form->create('vendors', array('action' => 'listmasterspurchase', 'type' => "GET"), array('class' => "control-group")); ?>
                <?php
                        ...
                        if (isset($data) && !empty($data)) {
                            foreach ($data as $key => $processData) {
                                ...
                                <tr>
                                    ...
                                    <td><?php echo $processData['VendorMasterPurchase']['material']; ?></td>
                                    <td align="center"><?php echo $processData['VendorMasterPurchase']['quantity']; ?></td>
                                    <td><?php echo date("d-m-Y", strtotime($processData['VendorMasterPurchase']['added_date'])); ?></td>
                                    <td><?php echo $processData['User']['full_name']."(".$processData['UserType']['name'].")"; ?></td>
                                    <!--td><?php // echo ($processData['VendorMasterPurchase']['status'] == 1) ? '<span class="label label-success">Active</span>' : '<span class="label label-danger">Inactive</span>'; ?></td-->
                                    <td><?php echo date(Configure::read('DISPLAY_DATETIME.DateTime'), strtotime($processData['VendorMasterPurchase']['created'])); ?></td>
                                    <td>
                                        <?php echo $this->Html->link('<i class="glyphicon glyphicon-search text-color"></i>', array('controller' => 'vendors', 'action' => 'viewmasterpurchase', base64_encode($processData['VendorMasterPurchase']['id']), 'admin' => true), array('escape' => false, 'class' => 'btn btn-sm btn-default viewUserRow', 'title' => 'View master details',)); ?>
                                        <?php // echo $this->Html->link('<i class="glyphicon glyphicon-pencil text-color"></i>', array('controller' => 'users', 'action' => 'editadmin', base64_encode($processData['VendorMasterPurchase']['id'])), array('escape' => false, 'class' => 'btn btn-sm btn-default editRow', 'title' => 'Edit User')); ?>				
                                        <?php
                                        /*if ($processData['VendorMasterPurchase']['status'] == 1) {
                                            echo $this->Html->link('<i class="glyphicon glyphicon-ok-circle"></i>', array('controller' => 'users', 'action' => 'blockadmin', base64_encode($processData['VendorMasterPurchase']['id']), 'block'), array('data-username' => $processData['VendorMasterPurchase']['first_name'] . ' ' . $processData['VendorMasterPurchase']['last_name'], 'escape' => false, 'class' => 'btn btn-sm btn-default block', 'title' => 'Deactivate Administrator'));
                                        } else {
                                            echo $this->Html->link('<i class="glyphicon glyphicon-ban-circle"></i>', array('controller' => 'users', 'action' => 'blockadmin', base64_encode($processData['VendorMasterPurchase']['id']), 'unblock'), array('data-username' => $processData['VendorMasterPurchase']['first_name'] . ' ' . $processData['VendorMasterPurchase']['last_name'], 'escape' => false, 'class' => 'btn btn-sm btn-default block', 'title' => 'Activate Administrator'));
                                        }*/
                                        ?>
                                        <?php // echo $this->Html->link('<i class="glyphicon glyphicon-trash text-color"></i>', 'javascript:void(0)', array('data-name' => $processData['VendorMasterPurchase']['first_name'] . ' ' . $processData['VendorMasterPurchase']['last_name'], 'escape' => false, 'class' => 'btn btn-sm btn-default delete', 'title' => 'Delete User', 'data-href' => $this->Html->url(array('controller' => 'users', 'action' => 'deleteadmin', base64_encode($processData['VendorMasterPurchase']['id']))))); ?>
                                    </td> 
                                </tr>
                                <?php
                            }
...
<?php echo $this->Element('admin/addVendorMasterPurchase'); ?>
<!-- Modal Edit Client -->
<?php echo $this->Element('admin/viewVendorMasterPurchase'); ?>
